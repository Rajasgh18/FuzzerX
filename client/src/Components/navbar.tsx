...
function Navbar({ showSearchBar = false }) {
  ...
  return (
    <nav className="p-4 flex bg-[#141622] justify-between items-center">
      <div className="flex items-center space-x-4">
        <Link to="/" className="text-white text-4xl font-bold">
          Fuzzer<span className="text-[#838AC1]">X</span>
        </Link>
        <div className="relative">
          <button onClick={toggleDropdown} className="text-white font-medium py-2 px-4 bg-[#838AC1] rounded flex items-center space-x-2 ml-6">
          ...
          {dropdownOpen && (
            <ul className="absolute top-12 left-0 text-gray-900 rounded-md shadow-lg w-56 py-2 z-50">
              {Object.keys(pageNames).map((key) => (
                <li key={key} className="px-4 py-2 bg-[#515573] text-white cursor-pointer">
      ...
      {showSearchBar && <input type="text" placeholder="Search..." className="border text-white bg-[#141622] border-white rounded-lg py-2 px-4 w-1/3" />}
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="flex items-center m-2">
          ...
          />
          <span className="text-white bg-[#8491ED] font-medium rounded-lg ml-10 mr-2 px-6 py-2">Suraj Patel</span>
        </div>
      </div>
    </nav>
  );
